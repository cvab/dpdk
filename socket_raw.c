/* 
 *  @File: netlink_raw.c 
 *  @Description: Packet reflector application 
 */ 
...
uint16_t ip_csum(const void *buff, size_t len)
{
...
int main(int argc, char *argv[])
{
	...
	while (1) {	
		...
		if (eh->ether_type == htons(ETH_P_IP)) { 
			...
			iph->check = ip_csum(buf + sizeof(struct ether_header), sizeof(struct iphdr));  
		} 
		uint8_t mac_addr[6]; 
